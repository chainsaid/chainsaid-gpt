...
class Role(Plugin):
    ...
    def on_handle_context(self, e_context: EventContext):
        if e_context["context"].type != ContextType.TEXT:
            return
        if e_context["context"]["isgroup"] and not conf().get("roleplay_in_group", False):
            return
        ...
        if clist[0] == f"{trigger_prefix}停止扮演":
            ...
            desckey = "description"
        elif conf().get("role_customizable") and clist[0] == f"{trigger_prefix}设定扮演":
            customize = True
